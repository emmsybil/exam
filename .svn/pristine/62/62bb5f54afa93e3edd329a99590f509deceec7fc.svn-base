			...
			<div class="row">
				<div class="col-xs-2">
					<ul class="nav default-sidenav">
						...
						<li class="active"><a> <i class="fa fa-history"></i> 考试历史
						</a></li>
						<li><a href="student/training-history"> <i
								class="fa fa-book"></i> 培训历史
						</a></li>
						<li><a href="student/dictationCase"
										...
										<tr>
											...
											<td><c:choose>
													<c:when test="${item.approved == 0 }">
														<span class="label-warning label-sm label">未审核</span>
													</c:when>
													<c:when test="${item.approved == 1 }">
														<span class="label-default label-sm label">已审核</span>
													</c:when>
													<c:when test="${item.approved == 2 }">
														<span class="label-info label-sm label">已交卷</span>
													</c:when>
													<c:when test="${item.approved == 3 }">
														<span class="label-success label-sm label">已阅卷</span>
													</c:when>
												</c:choose></td>
											<td><c:choose>
											...
											<td><c:choose>
													<c:when
														test="${item.approved == 3 or (item.examType == 3 and item.approved != 1)}">
														<a class="btn btn-success btn-sm"
															href="student/student-answer-sheet/${item.examId }"
															target="_blank" title="预览">详细解答</a>
													</c:when>
												</c:choose></td>
										</tr>
									</c:forEach>
