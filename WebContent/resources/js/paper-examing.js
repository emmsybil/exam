...
var examing = {
	...
	bindFinishOne : function bindFinishOne() {
		$(".question input[type=radio]").change(
				function() {
					var current_index = $("li.question").index(
							$(this).parent().parent().parent().parent());
					$($("a.question-navi-item")[current_index]).addClass(
							"pressed");
					$(this).parent().parent().find(
							".question-list-item-selected").removeClass(
							"question-list-item-selected");
					$(this).parent().addClass("question-list-item-selected");
				});

		$(".question input[type=checkbox]").change(
				function() {
					var current_question = $(this).parent().parent().parent()
							.parent();
					var current_index = $("li.question")
							.index(current_question);
					var checkedboxs = current_question
							.find("input[type=checkbox]:checked");
					if (checkedboxs.length > 0) {
						$($("a.question-navi-item")[current_index]).addClass(
								"pressed");
					} else {
						$($("a.question-navi-item")[current_index])
								.removeClass("pressed");
					}

					if ($(this).parent()
							.hasClass("question-list-item-selected")) {
						$(this).parent().removeClass(
								"question-list-item-selected");
					} else {
						$(this).parent()
								.addClass("question-list-item-selected");
					}
				});

		$(".question textarea").bind(
	...
	bindNaviBehavior : function bindNaviBehavior() {
		 ...
		 * "px" // }); } else { // nav.removeClass("fixed-navi"); //
		 * nav.attr("style", ""); nav.css({ position : 'relative', bottom : 0
		 * }); }
		 * 
		 * });
		 */

		$("#question-navi-controller").click(
				function() {
	...
	refreshNavi : function refreshNavi() {
		...
		questions.each(function(index) {
			var btnhtml = "<a class=\"question-navi-item\">" + (index + 1)
					+ "</a>";
	...
	updateSummery : function updateSummery() {
		...
		for (var i = 0; i < questiontypes.length; i++) {
			...
			if (question_sum_q == 0) {
				...
				summery = summery
						+ "<span class=\"exampaper-filter-item efi-"
						+ questiontypes[i].code
						+ "\">"
						+ questiontypes[i].name
						+ "[<span class=\"efi-fno\">0</span>/<span class=\"efi-tno\">"
						+ $("." + questiontypes[i].code).length
						+ "</span>]<span class=\"efi-qcode\" style=\"display:none;\">"
		...
		examing.doQuestionFilt($($(".exampaper-filter-item")[0]).find(
				".efi-qcode").text());
	},
	...
	bindfocus : function bindfocus() {
		$("#question-navi").delegate(
				"a.question-navi-item ",
				"click",
				function() {
					...
					var targetQuestionType = $(questions[clickindex]).find(
							".question-type").text();
	...
	bindQuestionFilter : function bindQuestionFilter() {
		...
		$("#exampaper-desc").delegate("span.exampaper-filter-item", "click",
				function() {
	...
	finishExam : function finishExam() {
		...
		var request = $.ajax({
			...
			type : "POST",
			url : "student/exam-submit/" + $("#start-time").val(),
	...
	genrateAnswerSheet : function genrateAnswerSheet() {
		...
		for (var i = 0; i < questions.length; i++) {
			...
			if ($(questions[i]).hasClass("qt-singlechoice")) {
				var radio_checked = $(questions[i]).find(
				...
				if (checkbox_checked.length == 0) {
					...
					for (var l = 0; l < checkbox_checked.length; l++) {
						var current_index = $(checkbox_all).index(
								$(checkbox_checked[l]));
						tm_answer = tm_answer
								+ String.fromCharCode(65 + current_index);
					}
			...
			answerSheetItem.questionId = $(questions[i]).find(".question-id")
					.text();
			/*
			 * var tmpkey = $(questions[i]).find(".question-id").text(); var
			 * tmpvalue = answerSheetItem;
			 * 
			 * as[tmpkey] = tmpvalue;
			 */
	...
	mergeQuestionAnswer : function mergeQuestionAnswer(answerSheet) {
		if (answerSheet == null
				|| answerSheet.examHistroyId != $("#hist-id").val())
			return false;
		var questions = $("li.question");
		/*
		 * var questionMap = new Object();
		 * 
		 * for(var i=0;i<questions.length; i++){ var key =
		 * $(questions[i]).find(".question-id").text(); questionMap[key] =
		 * questions[i]; }
		 */
		...
		for (var i = 0; i < list.length; i++) {
			if (list[i].questionTypeId == 1) {
				...
				$(questions[i]).find("input[value='" + tmp_char + "']").prop(
						'checked', true);
				$(quetion_navis[i]).addClass("pressed");
				$(questions[i]).find("input[value='" + tmp_char + "']")
						.parent().addClass("question-list-item-selected");
				...
				function() {
					if ($(this).find(".fa-bookmark").length > 0) {
						...
						var current_index = $("li.question").index(
								$(this).parent().parent());
						$($("a.question-navi-item")[current_index])
								.removeClass("question-navi-item-marked");
...
var modal = {
	prepare : function prepare() {
		$(".content")
				.append(
						"<div id=\"loading-progress\" style=\"display:none;\"><div id=\"loading-content\"> <h2>正在提交您的答案</h2><img class=\"loading-gif\" src=\"resources/images/loading.gif\"/><div> </div>");
